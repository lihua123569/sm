		...
		this
				.each(function() {
					...
					function getNbCols() {
						...
						var colWidth = container.find(prm.columnClass)
								.outerWidth(true);
					...
					function print() {
						...
						for ( var i in elemsDatas) {
							...
							if (elemsDatas[i].href != undefined) {
								html += '<a ' + getAttr(href, 'href') + ' '
										+ getAttr(classe, 'class') + ' '
										+ getAttr(id, 'id') + '>' + content
										+ '</a>';
							} else {
								html += '<div ' + getAttr(classe, 'class')
										+ ' ' + getAttr(id, 'id') + '>'
										+ content + '</a>';
							}
							container.children(prm.columnClass).eq(i % nbCols)
									.append(html);
						}
					}
